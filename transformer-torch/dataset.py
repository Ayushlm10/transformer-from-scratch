...
class BilingualDataset(Dataset):
    ...
    def __getitem__(self, index) -> Any:
        ...
        # add sos and eos tokens in front and back of each sentence for encoder
        ...
        assert encoder_input.size(0) == self.seq_len, (
            f"encoder input size is {encoder_input.size(0)} but should be {self.seq_len}"
        )
        assert decoder_input.size(0) == self.seq_len, (
            f"decoder input size is {decoder_input.size(0)} but should be {self.seq_len}"
        )
        assert label.size(0) == self.seq_len, (
            f"label size is {label.size(0)} but should be {self.seq_len}"
        )
        return {
